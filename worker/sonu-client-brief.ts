...
export type SonuClientNarrative = {
  headline: string;
...
export type SonuMarketEvidence = {
  title: string;
  summary: string;
  marketDate: string;
  sourceLabel: string;
...
function projectReportMedia(project: SonuRecommendation) {
...
async function enrichedProjectReportMedia(project: SonuRecommendation, env: BriefEnv) {
...
export async function renderSonuClientBriefPdf({
  id,
  clientName,
  preparedAt,
  preferences,
  ...
  narrative: SonuClientNarrative;
  marketEvidence?: SonuMarketEvidence | null;
  env: BriefEnv;
}) {
...
export function sonuClientBriefFilename(clientName: string) {
