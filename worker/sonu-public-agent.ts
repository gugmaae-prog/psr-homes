...
import {
  ...
  sonuPropertyBudgetCombinationIsValid,
  sonuPropertyBedroomCombinationIsValid,
  sonuQuickReplies,
  type SonuChatMessage,
  type SonuDiscoveryProfile,
  type SonuDiscoveryQuestion,
} from "../lib/sonu-chat";
import {
...
import {
  generateSonuClientBrief,
  sonuFinderPreferencesFromDiscovery,
} from "./sonu-brief-service";
...
type SonuState = {
  messages: SonuChatMessage[];
  briefMessages: SonuChatMessage[];
...
function projectMeetsRecordedRequirements(
  ...
  profile: ReturnType<typeof extractSonuDiscoveryProfile>,
  decisionContext: SonuDecisionContext = EMPTY_SONU_DECISION_CONTEXT,
) {
  if (!sonuPropertyBedroomCombinationIsValid(profile.propertyType, profile.bedrooms)) return false;
  if (!sonuPropertyBudgetCombinationIsValid(profile.propertyType, profile.budget)) return false;
  if (profile.location) {
...
function sonuPurposeFitScore(project: ProjectRecord, profile: SonuDiscoveryProfile, query: string) {
...
export function sonuRankedProjectSuggestions(messages: SonuChatMessage[]) {
  ...
  if (sonuDiscoveryCompleteness(profile) < 4 && requirementSpecificity(messages) < 3) return [];
...
const SONU_RELAXABLE_CRITERIA: Array<{
  key: keyof Pick<SonuDiscoveryProfile, "location" | "propertyType" | "bedrooms" | "budget" | "timeline">;
  label: string;
}> = [
...
function compactConversation(messages: SonuChatMessage[]) {
...
function explicitAdvisorContinuation(messages: SonuChatMessage[]) {
...
function latestUserMessage(messages: SonuChatMessage[]) {
...
function unitPriceQuickReplies(profile: ReturnType<typeof extractSonuDiscoveryProfile>) {
...
function resultReply(
  projects: ProjectSuggestion[],
  messages: SonuChatMessage[],
  question: ReturnType<typeof sonuDiscoveryNextQuestion>,
...
function isFreshReply(candidate: string, messages: SonuChatMessage[]) {
...
function mentionsDiscoveryTopic(candidate: string, question: SonuDiscoveryQuestion) {
  const patterns: Record<SonuDiscoveryQuestion["key"], RegExp> = {
...
function conversationalDiscoveryReply(
  ...
  messages: SonuChatMessage[],
  question: SonuDiscoveryQuestion,
) {
...
export function wantsProjectSelection(message: string) {
  ...
  const profile = extractSonuDiscoveryProfile([{ role: "user", content: message }]);
...
export function sonuSocialReply(message: string, agentName = "") {
  const normalized = clean(message, MAX_MESSAGE_LENGTH)
    .toLowerCase()
    .replace(/[.!?]+$/g, "")
    .trim();
  const addressed = normalized
    .replace(/^(?:(?:sonu|sonu)[\s,:-]+)+/i, "")
    .replace(/(?:[\s,:-]+(?:sonu|sonu))$/i, "")
    .trim();
...
function conciseQuestionPrompt(question: SonuDiscoveryQuestion) {
  switch (question.key) {
    case "purpose":
...
function sonuStartsFreshConversation(message: string) {
  const normalized = clean(message, MAX_MESSAGE_LENGTH)
    .toLowerCase()
    .replace(/[.!?]+$/g, "")
    .trim()
...
export function shouldStartNewSonuBrief(previousBrief: SonuChatMessage[], message: string) {
  ...
  const previous = extractSonuDiscoveryProfile(previousBrief);
  const incoming = extractSonuDiscoveryProfile([{ role: "user", content: message }]);
  const conflicts = ([
...
export function parseAiResponse(value: unknown): SonuAiResponse | null {
  ...
  try {
    if (value && typeof value === "object") {
      const parsed = value as Partial<SonuAiResponse>;
    ...
    const parsed = JSON.parse(start >= 0 && end > start ? withoutFence.slice(start, end + 1) : withoutFence) as Partial<SonuAiResponse>;
    const reply = clean(parsed.reply, 1_200);
    const knowledgeRoutes = parsedRoutes(parsed.knowledgeRoutes);
    return reply ? { reply, leadIntent: Boolean(parsed.leadIntent), ...(knowledgeRoutes.length ? { knowledgeRoutes } : {}) } : null;
  } catch {
...
function requirementSpecificity(messages: SonuChatMessage[]) {
...
export class SonuPublicAgent {
  ...
  private async readState() {
    const stored = await this.ctx.storage.get<Partial<SonuState>>("state");
  ...
  private response(
    ...
    const profile = extractSonuDiscoveryProfile(state.briefMessages);
    const nextQuestion = sonuDiscoveryNextQuestion(profile, state.lastQuestionKey);
    return Response.json({
      messages: state.messages,
      leadCaptured: state.leadCaptured,
      leadId: state.leadId,
      suggestions: state.suggestions,
      quickReplies: state.messages.length ? sonuQuickReplies(nextQuestion, profile) : [],
      filterIntent: sonuFilterIntent(profile),
      agent,
  ...
  private async saveLead(messages: SonuChatMessage[], projectReference: string) {
    const details = extractSonuLeadDetails(messages);
    ...
    if (this.env.EMAIL) {
      ...
      deliveries.forEach((delivery, index) => {
        ...
        console.error(JSON.stringify({
          event: "sonu_lead_notification_failed",
          leadId: inserted.id,
    ...
    await this.env.DB.prepare(
      "UPDATE haus_sonu_leads SET status = ? WHERE id = ?",
    ).bind(notified ? "notified" : "notification_failed", inserted.id).run();
    console.log(JSON.stringify({
      event: "sonu_lead_captured",
      leadId: inserted.id,
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    const agentEmail = clean(request.headers.get("x-sonu-agent-email"), 180).toLowerCase();
    const agentName = clean(request.headers.get("x-sonu-agent-name"), 100);
    const agentTitle = clean(request.headers.get("x-sonu-agent-title"), 100);
    ...
    const userMessage: SonuChatMessage = { role: "user", content: message, createdAt: new Date(now).toISOString() };
    const messages = compactConversation([...state.messages, userMessage]).slice(-MAX_MESSAGES);
    const socialReply = sonuSocialReply(message, agent?.name);
    const freshConversation = Boolean(socialReply && sonuStartsFreshConversation(message));
    const startsNewBrief = !socialReply && shouldStartNewSonuBrief(state.briefMessages, message);
    ...
    const profile = extractSonuDiscoveryProfile(briefMessages);
    const decisionContext = extractSonuDecisionContext(briefMessages, registryDeveloperNames);
    const invalidPropertyBedrooms = !sonuPropertyBedroomCombinationIsValid(profile.propertyType, profile.bedrooms);
    const invalidPropertyBudget = !sonuPropertyBudgetCombinationIsValid(profile.propertyType, profile.budget);
    ...
    const longTermMemory = agentEmail
      ...
      : null;
    const completeness = sonuDiscoveryCompleteness(profile);
    const primaryDiscoveryQuestion = sonuDiscoveryNextQuestion(profile);
    const discoveryQuestion = sonuDiscoveryNextQuestion(profile, startsNewBrief ? "" : state.lastQuestionKey);
    const harmlessDetourReply = sonuHarmlessDetourReply(message, primaryDiscoveryQuestion || discoveryQuestion);
    const details = extractSonuLeadDetails(briefMessages);
    const finderPreferences = sonuFinderPreferencesFromDiscovery(profile);
    ...
    let aiResponse: SonuAiResponse | null = null;
    if (
      this.env.AI
      && !socialReply
      && !harmlessDetourReply
      ...
      try {
        ...
        if (candidate && !aiResponse) {
          console.warn(JSON.stringify({ event: "sonu_ai_response_rejected", reason: "sales_or_grounding_guardrail" }));
        }
      } catch (error) {
        console.error(JSON.stringify({
          event: "sonu_ai_response_failed",
    ...
    const contactQuestion = wantsAdvisor && completeness >= 3
      ? sonuLeadNextQuestion(details, intentDetected)
      : "";
    ...
    if (!socialReply && !leadCaptured && sonuLeadIsQualified(details, intentDetected)) {
      try {
        leadId = await this.saveLead(briefMessages, matchedProject?.slug || "");
        leadCaptured = true;
      } catch (error) {
        console.error(JSON.stringify({
          event: "sonu_lead_capture_failed",
    ...
    if (
      ...
      try {
        const generated = await generateSonuClientBrief({
          env: this.env,
          leadId,
    ...
    if (!socialReply && !activeReportRequest && sonuLeadIsQualified(details, intentDetected) && !leadCaptured) {
    ...
    return this.response(updatedState, {
      ...
      quickReplies: socialReply
        ...
        : priceChallenge
          ...
          : sonuQuickReplies(harmlessDetourReply ? primaryDiscoveryQuestion || discoveryQuestion : discoveryQuestion, profile),
      filterIntent: socialReply
        ? { query: "", emirate: "", propertyType: "", bedrooms: "", budget: "" }
        : sonuFilterIntent(profile),
...
// Temporary compatibility alias for the existing Cloudflare Durable Object namespace.
// Remove only after the live class migration from GracePublicAgent to SonuPublicAgent is verified.
