...
import {
  ...
  type SonuMarketEvidence,
} from "./sonu-client-brief";
...
type SonuBriefEnv = Env & MediaStorageEnv & {
  AI?: Ai;
  EMAIL?: SendEmail;
...
function budgetFromAmount(value: string): SonuFinderPreferences["budget"] | "" {
...
function timelineFromProfile(value: string): SonuFinderPreferences["timeline"] {
...
export function sonuFinderPreferencesFromDiscovery(profile: SonuDiscoveryProfile): SonuFinderPreferences | null {
  ...
  if (
    ...
    || !profile.bedrooms
    || !sonuPropertyBedroomCombinationIsValid(profile.propertyType, profile.bedrooms)
    || !budget
    || !profile.location
    || !profile.timeline
  ) return null;
  const lifestyle: SonuFinderPreferences["lifestyle"] = /beach|waterfront|marina|palm|island/i.test(profile.location)
    ? "beachfront"
    : /golf|wellness/i.test(profile.location)
      ...
      : profile.purpose === "home"
        ...
        : /downtown|business bay|difc|central/i.test(profile.location)
          ? "central"
          : "full-community";
  const investmentStrategy: SonuFinderPreferences["investmentStrategy"] = /rental income/i.test(profile.investmentPriority)
    ? "income"
...
export function validatedSonuNarrativePriorities(value: unknown, fallback: string[]) {
...
async function createNarrative(
  ...
  try {
    ...
    const parsed = typeof result.response === "string"
      ? JSON.parse(result.response.replace(/^```(?:json)?\s*/i, "").replace(/\s*```$/i, "")) as Partial<SonuClientNarrative>
      : result.response as Partial<SonuClientNarrative> | undefined;
    return {
      ...
      validationPriorities: validatedSonuNarrativePriorities(
...
async function latestMarketEvidence(env: SonuBriefEnv, preferences: SonuFinderPreferences): Promise<SonuMarketEvidence | null> {
...
async function storePrivateBrief(env: SonuBriefEnv, leadId: number, clientName: string, pdf: Uint8Array) {
...
function clientEmail(name: string, recommendations: SonuRecommendation[]) {
...
export async function generateSonuClientBrief({
  env,
  leadId,
  clientName,
  email,
  ...
  const recommendations = recommendSonuProjectsStrict(preferences, 4);
  ...
  const pdf = await renderSonuClientBriefPdf({
    id: `PSR-SONU-${leadId}`,
    clientName,
...
export async function pruneExpiredSonuClientBriefs(env: SonuBriefEnv) {
