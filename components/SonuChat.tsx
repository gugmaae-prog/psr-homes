...
import type { SonuChatMessage, SonuFilterIntent, SonuQuickReply } from "@/lib/sonu-chat";
...
type ChatResponse = {
  ...
  quickReplies?: SonuQuickReply[];
  filterIntent?: SonuFilterIntent;
...
const OPEN_KEY = "psr:sonu-chat-open:v1";
let documentSessionId = "";
const GREETING: SonuChatMessage = {
  role: "assistant",
...
function conciseConversation(messages: SonuChatMessage[]) {
...
function greetingFor(agent: ChatResponse["agent"]): SonuChatMessage {
...
export function SonuChat({ initialOpen = false }: { initialOpen?: boolean } = {}) {
  const [open, setOpen] = useState(initialOpen);
  const [messages, setMessages] = useState<SonuChatMessage[]>([GREETING]);
  const [suggestions, setSuggestions] = useState<ProjectSuggestion[]>([]);
  const [quickReplies, setQuickReplies] = useState<SonuQuickReply[]>([]);
  ...
  function syncPageFilters(filterIntent?: SonuFilterIntent) {
    ...
    window.dispatchEvent(new CustomEvent("psr:sonu-filter", { detail: filterIntent }));
  ...
  useEffect(() => {
    ...
    fetch("/api/sonu-chat", {
      headers: { "x-sonu-session": sessionId() },
      cache: "no-store",
    })
      .then(async (response) => {
  ...
  useEffect(() => {
    window.dispatchEvent(new CustomEvent("hg:overlay-change", {
      detail: { source: "sonu-chat", open },
    }));
    return () => {
      if (open) {
        window.dispatchEvent(new CustomEvent("hg:overlay-change", {
          detail: { source: "sonu-chat", open: false },
        }));
      }
    };
  }, [open]);
  ...
  useEffect(() => {
    function handleOverlayChange(event: Event) {
      ...
      if (!open || !detail?.open || detail.source === "sonu-chat") return;
      rememberPanelState(false);
      setOpen(false);
    }
  ...
  async function sendMessage(value: string) {
    ...
    try {
      ...
      for (let attempt = 0; attempt < 2 && !result; attempt += 1) {
        ...
        try {
          const response = await fetch("/api/sonu-chat", {
            method: "POST",
            headers: {
              "content-type": "application/json",
              "x-sonu-session": sessionId(),
            },
  ...
  function openGuidedFinder() {
    setPanelOpen(false);
    window.setTimeout(() => {
      window.dispatchEvent(new CustomEvent("hg:open-sonu-finder"));
    }, 120);
  }
  ...
  return (
    <div className={`sonu-chat ${open ? "is-open" : ""} ${loading ? "is-thinking" : ""} is-${expression}`}>
      {open ? (
        <section className="sonu-chat-panel" role="dialog" aria-modal="false" aria-label="Chat with Sonu, PSR AI property concierge">
          <header className="sonu-chat-header">
            <div className="sonu-chat-identity">
              <span className="sonu-chat-monogram sonu-chat-avatar" aria-hidden="true">
            ...
            <button type="button" className="sonu-chat-close" aria-label="Close Sonu" onClick={() => setPanelOpen(false)}>
              <span aria-hidden="true" />
            </button>
          </header>

          <div className="sonu-chat-messages" ref={messagesRef} aria-live="polite">
            {messages.map((message, index) => (
              <p className={`sonu-chat-message is-${message.role}`} key={`${message.role}-${index}-${message.content.slice(0, 20)}`}>
                {message.content}
              </p>
            ))}
            {loading ? <div className="sonu-chat-progress" role="status">
              <span className="sonu-chat-progress-mark" aria-hidden="true"><i /></span>
              <span>
            ...
            {error ? <p className="sonu-chat-error" role="alert">{error}</p> : null}
            {leadCaptured ? (
              <p className="sonu-chat-captured">
                <span aria-hidden="true" />
                Sent securely to the PSR advisory desk
              </p>
            ) : null}
            {suggestions.length && !loading ? (
              <div className="sonu-chat-suggestions" aria-label="Relevant projects">
                {suggestions.map((project) => (
                  ...
                  >
                    {project.image ? <span className="sonu-chat-project-image"><img src={project.image} alt="" width="320" height="240" loading="lazy" decoding="async" /></span> : null}
                    <span className="sonu-chat-project-copy">
                      <small>{project.developer || "PSR selection"}</small>
                      <strong>{project.name}</strong>
                      <span>{project.location}</span>
                      <span className="sonu-chat-price-scope">Project entry · {project.startingPrice}</span>
                      <span>{[project.paymentPlan, project.handover].filter(Boolean).join(" · ")}</span>
                    </span>
                    <span className="sonu-chat-project-open">Open project <i aria-hidden="true" /></span>
                  </a>
                ))}
              </div>
            ) : null}
          ...
          <div className="sonu-chat-actions">
            {messages.length === 1 ? (
              <div className="sonu-chat-prompts" aria-label="Conversation starters">
            ...
            {quickReplies.length && !loading ? (
              <div className="sonu-chat-quick-replies" aria-label="Choose an answer">
            ...
            {articles.length && !loading ? (
              <div className="sonu-chat-articles" aria-label="Relevant market research">
            ...
            {agentAction ? (
              <a className="sonu-chat-agent-action" href={agentAction.href} onClick={prepareForNativeNavigation}>
                {agentAction.label}
              </a>
            ) : null}
            {briefAction ? (
              <button className="sonu-chat-brief-cta" type="button" onClick={runBriefAction}>
                {briefAction.label}
              </button>
            ) : null}
            {leadCaptured && !briefAction ? (
              <button className="sonu-chat-brief-cta" type="button" onClick={openGuidedFinder}>
                Create my private PDF
              </button>
            ) : null}
          </div>

          <form className="sonu-chat-form" onSubmit={submit}>
            <label className="sr-only" htmlFor="sonu-chat-input">Message Sonu</label>
            <input
              ref={inputRef}
              id="sonu-chat-input"
              value={draft}
          ...
          <p className="sonu-chat-disclosure">AI guidance. Prices and availability require advisor confirmation.</p>
        </section>
      ) : null}
