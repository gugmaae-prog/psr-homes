...
import { gracePropertyBedroomCombinationIsValid } from "@/lib/grace-chat";
...
const INITIAL: GraceFinderPreferences = {
  goal: "unsure",
  propertyTypes: [],
  bedrooms: "",
  household: "not-sure",
...
function finderResponse(value: unknown): FinderResponse {
  ...
  return {
    ...
    recommendations: Array.isArray(record.recommendations)
      ? record.recommendations.filter((item): item is GraceRecommendation => {
...
function choiceClass(active: boolean) {
  return active ? "grace-choice is-active" : "grace-choice";
}

export function GraceFinder({ initialOpen = false }: { initialOpen?: boolean } = {}) {
  const [open, setOpen] = useState(false);
  const [step, setStep] = useState(0);
  const [answers, setAnswers] = useState<GraceFinderPreferences>(INITIAL);
  const [status, setStatus] = useState<"idle" | "sending" | "success" | "error">("idle");
  const [error, setError] = useState("");
  const [recommendations, setRecommendations] = useState<GraceRecommendation[]>([]);
  ...
  const announceOverlay = useCallback((isOpen: boolean) => {
    window.dispatchEvent(new CustomEvent("hg:overlay-change", {
      detail: { source: "grace-finder", open: isOpen },
    }));
  }, []);
  ...
  useEffect(() => {
    ...
    const handleOverlay = (event: Event) => {
      ...
      if (detail?.source === "grace-finder") return;
    ...
    window.addEventListener("hg:open-grace-finder", handleOpenRequest);
    return () => {
      ...
      window.removeEventListener("hg:open-grace-finder", handleOpenRequest);
    };
  }, [openFinder]);
  ...
  function toggleType(value: string) {
    setAnswers((current) => {
      ...
      return {
        ...current,
        propertyTypes,
        bedrooms: gracePropertyBedroomCombinationIsValid(propertyTypes, current.bedrooms)
          ? current.bedrooms
          : "",
      };
    });
    ...
    try {
      const response = await fetch(withBasePath("/api/leads"), {
        ...
        body: JSON.stringify({
          ...
          source: "grace-finder",
          deliveryType: "grace_finder",
          preferences: answers,
          graceSessionId: (() => {
            try {
              const key = "hg:grace-chat-session:v2";
      ...
      setStatus("success");
      setStep(7);
      try { window.sessionStorage.setItem("hg:grace-finder:completed", "yes"); } catch { /* device storage is optional */ }
      window.dispatchEvent(new CustomEvent("hg:lead-submitted", {
        detail: { source: "grace-finder", deliveryType: "grace_finder" },
      }));
  ...
  return <>
    <div className="grace-finder-modal" hidden={!open} onMouseDown={handleBackdrop}>
      <section
        ref={dialogRef}
        className="grace-finder-dialog"
        role="dialog"
        aria-modal="true"
        aria-labelledby="grace-finder-title"
      >
        <button ref={closeRef} className="grace-finder-close" type="button" onClick={closeFinder} aria-label="Close PSR Property Finder"><span aria-hidden="true" /></button>
        <aside className="grace-finder-aside">
          <p>PSR Property Finder</p>
          <h2 id="grace-finder-title">Don&rsquo;t know what you are looking for?</h2>
          ...
          <div className="grace-finder-progress" aria-label={`Step ${Math.min(step + 1, 7)} of 7`}>
            <span style={{ width: `${Math.min(100, ((step + 1) / 7) * 100)}%` }} />
          </div>
        </aside>
        <div className="grace-finder-content">
          {step === 0 && <div className="grace-finder-step">
            <p className="grace-step-number">01 · Your goal</p>
            <h3>What should this property do for you?</h3>
            <div className="grace-choice-grid">
              {([
          ...
          {step === 1 && <div className="grace-finder-step">
            <p className="grace-step-number">02 · Residence</p>
            <h3>Which property types feel right?</h3>
            <p className="grace-step-note">Choose one or more. The shortlist can compare different formats.</p>
            <div className="grace-choice-grid grace-choice-grid-compact">
          ...
          {step === 2 && <div className="grace-finder-step">
            <p className="grace-step-number">03 · Space and household</p>
            <h3>How much space should the unit support?</h3>
            <div className="grace-pill-grid">
              {BEDROOMS.filter((value) => gracePropertyBedroomCombinationIsValid(answers.propertyTypes, value)).map((value) => <button key={value} type="button" className={choiceClass(answers.bedrooms === value)} aria-pressed={answers.bedrooms === value} onClick={() => setAnswers((current) => ({ ...current, bedrooms: value }))}>{value}</button>)}
            </div>
          ...
          {step === 3 && <div className="grace-finder-step">
            <p className="grace-step-number">04 · Setting</p>
          ...
          {step === 4 && <div className="grace-finder-step">
            <p className="grace-step-number">05 · Priorities</p>
            ...
            <p className="grace-step-note">Select up to five priorities. These will shape the advisor&rsquo;s validation work.</p>
            <div className="grace-choice-grid grace-choice-grid-compact">
            ...
            <div className="grace-logic-note">
              {investorFocused
          ...
          {step === 5 && <div className="grace-finder-step">
            <p className="grace-step-number">06 · Budget and timing</p>
            ...
            <div className="grace-select-grid">
            ...
            <label className="grace-consent grace-visa-interest"><input type="checkbox" checked={Boolean(answers.goldenVisaInterest)} onChange={(event) => setAnswers((current) => ({ ...current, goldenVisaInterest: event.target.checked }))} /><span>Include the UAE Golden Visa property pathway in my brief.</span></label>
          </div>}

          {step === 6 && <form className="grace-finder-step grace-contact" onSubmit={submit}>
            <p className="grace-step-number">07 · Your shortlist</p>
            <h3>Where should we send your private client brief?</h3>
            <p className="grace-step-note">Sonu will create a private PDF after you submit this form. You will receive a time-limited download; email delivery follows when the company mailbox is active.</p>
            ...
            <label className="grace-consent"><input name="consent" type="checkbox" value="yes" required /><span>I agree to be contacted about this property search and accept the privacy policy.</span></label>
            <button className="grace-submit" type="submit" disabled={status === "sending"}>{status === "sending" ? "Preparing your private brief..." : "Create my private PDF"}</button>
            <p className="grace-error" aria-live="polite">{status === "error" ? error : ""}</p>
          </form>}

          {step === 7 && <div className="grace-finder-step grace-results">
            <p className="grace-step-number">Your considered matches</p>
            <h3>Your private brief is ready.</h3>
            <p className="grace-step-note">{briefSent ? "The PDF has also been sent to your email. " : "Email delivery will become available when the PSR company mailbox is activated. "}These matches are a considered starting point; an advisor will verify live units, exact pricing and current evidence.</p>
            {briefDownloadUrl ? <a className="grace-download-brief" href={withBasePath(briefDownloadUrl)} download>Download private PDF <span>Available for 24 hours</span></a> : null}
            <div className="grace-result-list">
              {recommendations.map((project) => <a key={project.slug} href={withBasePath(`/projects/${project.slug}`)}>
                {project.image && <div className="grace-result-image" style={{ backgroundImage: `url("${project.image.replaceAll('"', "%22")}")` }} aria-hidden="true" />}
                <div><span>{project.area} · {project.price}</span><strong>{project.title}</strong><p>{project.reason}</p></div>
              </a>)}
            </div>
            <button className="grace-submit" type="button" onClick={closeFinder}>Continue browsing</button>
          </div>}

          {step < 6 && <div className="grace-step-actions">
