...
import {
  detectGraceContactConsent,
  detectGraceLeadIntent,
...
import {
  ...
  shouldStartNewGraceBrief,
  unitPriceChallengeReply,
  wantsClientReport,
  wantsKnowledgeAnswer,
  wantsProjectSelection,
  type ProjectRecord,
} from "../worker/grace-public-agent";
import { graceFinderPreferencesFromDiscovery } from "../worker/grace-brief-service";
import {
  psrCatalogueContext,
...
test("detects UAE property buying and investment intent", () => {
  assert.equal(detectGraceLeadIntent("I am looking for a two bedroom apartment in Dubai"), true);
  assert.equal(detectGraceLeadIntent("Find a family home"), true);
  assert.equal(detectGraceLeadIntent("What are your office hours?"), false);
});

test("extracts contact details without treating an AED budget as a phone", () => {
  const details = extractGraceLeadDetails([
  ...
  assert.equal(graceLeadIsQualified(details), true);
});
...
test("requires explicit permission before a detected lead is uploaded", () => {
  ...
  assert.equal(graceLeadIsQualified(details), false);
  assert.match(graceLeadNextQuestion(details), /May a PSR advisor contact you/);
});

test("accepts clear contact consent", () => {
  assert.equal(detectGraceContactConsent("Yes, an advisor may contact me."), true);
  assert.equal(detectGraceContactConsent("I am only browsing."), false);
});
...
test("builds a progressive property brief without repeating answered questions", () => {
  ...
  assert.equal(graceDiscoveryCompleteness(profile), 5);
  assert.equal(graceDiscoveryNextQuestion(profile, "bedrooms")?.key, "timeline");
});

test("maps a complete Sonu discovery brief into strict report preferences", () => {
  const profile = extractGraceDiscoveryProfile([
  ...
  const preferences = graceFinderPreferencesFromDiscovery(profile);
...
test("moves to another missing field instead of repeating the previous question", () => {
  ...
  const question = graceDiscoveryNextQuestion(profile, "bedrooms");
...
test("corrects impossible one-bedroom landed-home requests before matching projects", () => {
  ...
  const question = graceDiscoveryNextQuestion(profile);
  ...
  assert.deepEqual(graceQuickReplies(question, profile).map((reply) => reply.label), [
    "2 bedrooms",
    "3 bedrooms",
    "4 bedrooms",
    "5+ bedrooms",
  ]);
  assert.equal(graceFinderPreferencesFromDiscovery(profile), null);
});

test("corrects implausible mansion bedroom and budget combinations deterministically", () => {
  const initial = extractGraceDiscoveryProfile([
    { role: "user", content: "I want a one-bedroom mansion in Dubai for AED 500,000." },
  ]);
  const bedroomQuestion = graceDiscoveryNextQuestion(initial);
  ...
  assert.deepEqual(graceQuickReplies(bedroomQuestion, initial).map((reply) => reply.label), [
    "4 bedrooms",
    "5+ bedrooms",
  ]);

  const correctedBedrooms = extractGraceDiscoveryProfile([
    { role: "user", content: "I want a one-bedroom mansion in Dubai for AED 500,000." },
    { role: "user", content: "Make that four bedrooms." },
  ]);
  const budgetQuestion = graceDiscoveryNextQuestion(correctedBedrooms);
  ...
  assert.deepEqual(graceQuickReplies(budgetQuestion, correctedBedrooms).map((reply) => reply.label), [
    "AED 5–10m",
    "AED 10m+",
  ]);
  assert.deepEqual(graceFilterIntent(correctedBedrooms), {
    query: "4 bedrooms",
    emirate: "Dubai",
  ...
  assert.equal(graceFinderPreferencesFromDiscovery(correctedBedrooms), null);
});
...
test("provides selectable answers and a page-filter intent from the conversation", () => {
  ...
  assert.deepEqual(graceQuickReplies(question).map((reply) => reply.label), [
    "Immediately",
    "Within 3 months",
    "Within 6 months",
    "This year",
  ...
  assert.deepEqual(graceFilterIntent(profile), {
    query: "2 bedrooms",
    emirate: "Dubai",
...
test("keeps greetings conversational without treating property requests as small talk", () => {
  assert.equal(graceSocialReply("hi"), "Hello. Good to meet you. What are you hoping to find?");
  assert.equal(graceSocialReply("Hi Grace!"), "Hello. Good to meet you. What are you hoping to find?");
  assert.equal(graceSocialReply("hello", "Mehul Kumar"), "Hello, Mehul. Good to see you. What are you working through today?");
  assert.equal(graceSocialReply("how are you?"), "I’m well, thank you. What are you hoping to find or work through today?");
  assert.equal(graceSocialReply("I'm good, thanks"), "Glad to hear it. What are you hoping to find?");
  assert.equal(graceSocialReply("Good, and you?"), "I’m well too, thank you. What are you hoping to find?");
  assert.equal(graceSocialReply("Hi, I need a two-bedroom apartment"), "");
});
...
test("starts a fresh property brief when the visitor changes direction", () => {
  ...
  assert.equal(shouldStartNewGraceBrief(previous, "Actually, show me 4-bedroom villas in Abu Dhabi instead."), true);
  assert.equal(shouldStartNewGraceBrief(previous, "What is the payment plan for those options?"), false);
  assert.equal(shouldStartNewGraceBrief(previous, "Start over with a new search"), true);
});
...
test("ranks recorded ready homes ahead of long-dated projects for an immediate family move", () => {
  ...
  const profile = extractGraceDiscoveryProfile([{ role: "user", content: query }]);
...
test("preserves a shortlist when the visitor says which one and recognises net-income intent", () => {
  ...
  const reply = contextualProjectReply(
    projects,
    messages,
    graceDiscoveryNextQuestion(profile),
    context,
  );
...
test("keeps quick replies aligned after a harmless Sonu detour", async () => {
  ...
  const agent = new GracePublicAgent({
    storage: {
  ...
  async function send(message: string) {
    const response = await agent.fetch(new Request("https://psrhomes.ae/api/grace-chat", {
      method: "POST",
