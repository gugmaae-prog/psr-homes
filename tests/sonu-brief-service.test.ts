...
import { normalizeSonuPreferences } from "../lib/sonu-finder";
import {
  generateSonuClientBrief,
  pruneExpiredSonuClientBriefs,
  validatedSonuNarrativePriorities,
} from "../worker/sonu-brief-service";
...
test("Sonu generates, stores and emails a grounded private PDF", async () => {
  ...
  const result = await generateSonuClientBrief({
    env: env as never,
    leadId: 42,
...
test("expired private brief tokens and R2 objects are pruned together", async () => {
  ...
  assert.equal(await pruneExpiredSonuClientBriefs(env as never), 2);
