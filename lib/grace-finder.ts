...
export type GraceGoal = "home" | "investment" | "holiday" | "unsure";
export type GraceLifestyle = "beachfront" | "full-community" | "central" | "golf-wellness" | "quiet-family";
export type GraceHousehold = "children" | "planning-family" | "adults" | "not-sure";
export type GraceBudget = "under-1m" | "1m-2m" | "2m-5m" | "5m-10m" | "10m-plus";
export type GraceFinancing = "cash" | "mortgage" | "mixed" | "not-sure";
export type GraceInvestmentStrategy = "income" | "growth" | "balanced";

export type GraceFinderPreferences = {
  goal: GraceGoal;
  propertyTypes: string[];
  bedrooms: string;
  household: GraceHousehold;
  lifestyle: GraceLifestyle;
  priorities: string[];
  budget: GraceBudget;
  emirate: string;
  community?: string;
  timeline: string;
  financing?: GraceFinancing;
  investmentStrategy?: GraceInvestmentStrategy;
  goldenVisaInterest?: boolean;
};

export type GraceRecommendation = {
  slug: string;
  title: string;
  developer: string;
  area: string;
...
const GOALS = new Set<GraceGoal>(["home", "investment", "holiday", "unsure"]);
const LIFESTYLES = new Set<GraceLifestyle>(["beachfront", "full-community", "central", "golf-wellness", "quiet-family"]);
const HOUSEHOLDS = new Set<GraceHousehold>(["children", "planning-family", "adults", "not-sure"]);
const BUDGETS = new Set<GraceBudget>(["under-1m", "1m-2m", "2m-5m", "5m-10m", "10m-plus"]);
...
const VALID_FINANCING = new Set<GraceFinancing>(["cash", "mortgage", "mixed", "not-sure"]);
const VALID_STRATEGIES = new Set<GraceInvestmentStrategy>(["income", "growth", "balanced"]);
...
const BUDGET_LIMITS: Record<GraceBudget, { min: number; max: number }> = {
...
export function normalizeGracePreferences(value: unknown): GraceFinderPreferences | null {
  if (!value || typeof value !== "object" || Array.isArray(value)) return null;
  const record = value as Record<string, unknown>;
  const goal = text(record.goal) as GraceGoal;
  const lifestyle = text(record.lifestyle) as GraceLifestyle;
  const household = text(record.household) as GraceHousehold;
  const budget = text(record.budget) as GraceBudget;
  ...
  const requestedFinancing = text(record.financing, 20) as GraceFinancing;
  const requestedStrategy = text(record.investmentStrategy, 20) as GraceInvestmentStrategy;
  ...
  if (
    ...
    || !gracePropertyBedroomCombinationIsValid(propertyTypes, bedrooms)
...
function lifestylePattern(lifestyle: GraceLifestyle) {
...
function preferenceSummary(preferences: GraceFinderPreferences, project: RegistryProject, price: number) {
  ...
  if (lifestylePattern(preferences.lifestyle).test(projectText(project))) {
    const labels: Record<GraceLifestyle, string> = {
...
function scoreProject(project: RegistryProject, preferences: GraceFinderPreferences) {
  const corpus = projectText(project);
...
export function recommendGraceProjects(preferences: GraceFinderPreferences, limit = 4): GraceRecommendation[] {
...
export function recommendGraceProjectsStrict(preferences: GraceFinderPreferences, limit = 4): GraceRecommendation[] {
  const budget = BUDGET_LIMITS[preferences.budget];
...
export function gracePreferenceMessage(preferences: GraceFinderPreferences) {
  return [
